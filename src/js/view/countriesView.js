        ...
        <a class="results__link" href="#${country.name}">
            <div class="results__card">
                <figure class="result__fig">
                    <img class="card-img-top" src="${country.flag}" alt="${
    country.name
  }">
                </figure>
                <div class="card-body">
                    <h3 class="results__name"> ${country.name}</h3>
                    <p class="results__population"><span>Population:</span>${country.population
                      .toFixed(2)
                      .replace(/\d(?=(\d{3})+\.)/g, "$&,")}</p>
                    <p class="results__region"><span>Region:</span> ${
                      country.region
                    }</p>
                    <p class="result__Capital"><span>Capital:</span> ${
                      country.capital
                    }</p>
                </div>
            </div>
...
const creatButton = (page, type) =>
  `
    ...
    type === "prev" ? page - 1 : page + 1
            ...
            <i class="fa fa-arrow-alt-circle-${
